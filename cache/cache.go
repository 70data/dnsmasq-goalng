...
// TODO(miek): there is a lot of copying going on to copy myself out of data races. This should be optimized.
...
// The signature is put in answer, extra is empty there.
// This wastes some memory.
type elem struct {
...
// The cache eviction is randomized.
type Cache struct {
	sync.RWMutex
	capacity int
	m        map[string]*elem
...
// InsertMessage inserts a message in the Cache.
// We will cache it for ttl seconds, which should be a small (60...300) integer.
...
// It creates a different key for requests with DNSSEC.
...
func KeyRRset(rrs []dns.RR) string {
	...
	for _, r := range rrs {
		// we only do a few type, serialize these manually
		switch t := r.(type) {
		case *dns.SOA:
