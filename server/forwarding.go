...
func (s *server) ServeDNSForward(w dns.ResponseWriter, req *dns.Msg) *dns.Msg {
	...
	switch {
	case s.config.NoRec:
		log.Printf("[%d] Refusing query, recursion disabled", req.Id)
		refuse = true
	...
	// responses from absolute/search lookups
	var absoluteRes, searchRes *dns.Msg
	// errors from absolute/search lookups
	var absoluteErr, searchErr error
	tcp := isTCP(w)
	if s.config.EnableSearch && len(s.config.SearchDomains) > 0 {
		searchEnabled = true
	}
	// If there are enough dots in the name, start with trying to resolve the literal name
	if nameDots >= s.config.Ndots {
		if nameDots >= s.config.FwdNdots {
			log.Printf("[%d] Doing initial absolute lookup", req.Id)
	...
	// We do at least one level of search if search is enabled and we didn't previously fail to query the upstreams
	...
	// If we didn't yet do an absolute lookup then do that now if there are enough dots in the name and searching did not previously fail
	...
	// If we did an absolute query, return that result, otherwise return a no-data response with the rcode from the last search we did.
	...
	// If we got here, we either failed to forward the query or the qname was too short to forward.
	// The caller needs to handle returning back an error
	return nil
}
...
func (s *server) forwardSearch(req *dns.Msg, tcp bool) (*dns.Msg, error) {
	var r *dns.Msg
	// stores the copy of a NODATA reply
	var nodata *dns.Msg
	// stores the current name suffixed with search domain
	...
	for _, domain := range s.config.SearchDomains {
		...
		case dns.RcodeSuccess:
			// In case of NO_DATA keep searching, otherwise a wildcard entry could keep us from finding the answer higher in the search list
...
func (s *server) forwardQuery(req *dns.Msg, tcp bool) (*dns.Msg, error) {
	// Nameservers to use for this query
	var nservers []string
	var nsIdx int
	var r *dns.Msg
	var err error
...
// ServeDNSReverse is the handler for DNS requests for the reverse zone.
// If nothing is found locally the request is forwarded to the forwarder for resolution.
