...
func (s *server) ServeDNS(w dns.ResponseWriter, req *dns.Msg) {
	...
	defer func() {
		// remove the expired cache entry if we're not in keep-working-if-upstream-goes-down mode, or if we got an upstream response.
		// We know m1 is expired since this defered function is running
	...
	if !local && resp == nil {
		// if we were unable to get a response (due to the upstream dns servers being down, etc.) fail back to the cached result we got in the beginning
...
func (s *server) RoundRobin(rrs []dns.RR) {
	...
	// If we have more than 1 CNAME don't touch the packet, because some stub resolver (=glibc) can't deal with the returned packet if the CNAMEs need to be accesses in the reverse order.
	cname := 0
	for _, r := range rrs {
